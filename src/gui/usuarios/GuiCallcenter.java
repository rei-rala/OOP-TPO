...
public class GuiCallcenter extends GuiUsuarioBase {
  ...
  private Cliente seleccionClienteDisponible() throws Exception {
    ...
    String opciones = "Seleccione cliente por su numero de DNI:";

    for (Cliente c : clientes) {
      opciones += "\n\t" + "DNI: " + c.getDni();
      opciones += ", Nombre: " + c.getNombre();
    }

    int nroSeleccionado = guiValidarInt(opciones);
    Cliente seleccionCliente = CALLCENTER.getClientesDNI(nroSeleccionado);

    if (seleccionCliente == null) {
      throw new ValorException("No existe el cliente");
    }

    if (clientes.contains(seleccionCliente) == false) {
      throw new ValorException("El cliente ya posee servicio activo");
    }
