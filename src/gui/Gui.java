...
public class Gui {
	...
	public String limpiarStrArticulos(ArrayList<Costo> alC) {
		...
		String costos = "";
		for (Costo c : alC) {
			costos += "\n\t" + c.getCantidad() + "x" + c.getArticulo().getDescripcion() + "= $" + c.obtenerTotalCosto();
	...
	public void adicionarCosto(Interno i, Servicio s) {
		try {
			...
			Articulo cable = empresa.getArticulo(Cable.class);
			Articulo conector = empresa.getArticulo(ConectorCoaxial.class);
			Articulo divisor = empresa.getArticulo(DivisorCoaxial.class);
			Articulo deco = empresa.getArticulo(DecodificadorTV.class);
			Articulo modem = empresa.getArticulo(Modem.class);
			...
			JOptionPane.showMessageDialog(null, "Añadido nuevo articulo " + getSimpleName(nuevoArticulo));
